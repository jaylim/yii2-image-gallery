...
namespace pentajeu\yii\behavior;
...
class GalleryBehavior extends Behavior
{
    ...
    {
        $owner->on({$owner->className()}::EVENT_BEFORE_INSERT, [$this, 'createGalleryIfNotExist']);
        $owner->on({$owner->className()}::EVENT_BEFORE_UPDATE, [$this, 'createGalleryIfNotExist']);
        // $owner->on({$owner->className()}::EVENT_BEFORE_DELETE, [$this, 'createGalleryIfNotExist']);
    }

    /** Will create new gallery after save if no associated gallery exists */
    public function createGalleryIfNotExist($event)
    {
